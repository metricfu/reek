module SmellOfMatcher
  class SmellOf
    ...
    def matches?(src)
      ...
      smell_repository = Core::SmellRepository.new(@source.desc, [@klass])
      smell_repository.configure @klass, @klass.default_config.merge(@config)

      sniffer = Core::Sniffer.new(@source, [], smell_repository)
      collector = Core::WarningCollector.new
      sniffer.report_on(collector)

      actual_smells = collector.warnings
